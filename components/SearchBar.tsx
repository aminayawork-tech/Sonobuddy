      ...
      <div className="relative">
        ...
        {query && (
          <button
            ...
            className="absolute right-3 top-1/2 -translate-y-1/2 text-sono-muted hover:text-slate-600 transition-colors"
          >
            <X className="w-4 h-4" />
          </button>
        )}
        ...
        <div className="absolute top-full mt-2 left-0 right-0 bg-sono-card border border-sono-border rounded-xl shadow-xl z-50 max-h-80 overflow-y-auto">
          {results.map((result) => (
            <button
              ...
              className="w-full flex items-start gap-3 px-4 py-3 hover:bg-slate-50 transition-colors text-left border-b border-sono-border last:border-0"
            >
