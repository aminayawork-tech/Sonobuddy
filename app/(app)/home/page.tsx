              ...
              <div className="bg-black rounded-t-3xl flex flex-col" style={{ maxHeight: '85vh' }}>
                ...
                <div className="overflow-y-auto px-5 py-4 space-y-4" style={{ paddingBottom: 'max(32px, env(safe-area-inset-bottom))' }}>
                  ...
                  {[
                    ...
                    { heading: '2. Information We Collect', body: 'No account required. You can use all core features without providing any personal information.\n\nUsage analytics: We record which screens are opened, which buttons are tapped and roughly where on the screen, how far down a screen is read, and the order screens are visited in. Each visit is grouped by a random identifier that is discarded when you close the app; it is not tied to you or your device and does not persist between sessions.\n\nSearches: If a search returns no results, we record the search term so we know what reference content to add. Searches that return results are not recorded, and terms that look like identifiers — anything containing an @ or a run of four or more digits — are discarded on your device before being sent.\n\nFeedback: If you use the Feedback form, we receive your message and, if you choose to provide it, your email address, solely to respond.\n\nWe do not collect: your medical records, patient data, precise location, financial information, contacts, or advertising identifiers. All clinical reference content is stored locally on your device.', jsx: null },
                    ...
                    { heading: '6. Sharing of Information', body: 'We do not sell, rent, or trade your personal information, and we do not use it for advertising or share it with data brokers. We do not track you across other apps or websites. Limited processing occurs with service providers acting on our behalf under their privacy terms — Vercel (hosting), Upstash (analytics storage), and Resend (delivering feedback email) — or if required by law.' },
                    { heading: '7. Local Storage', body: 'The app stores its reference content on your device so it works offline. That content never leaves your device.\n\nUsage events are also queued in local storage while you are offline and sent when a connection is available, so that scanning in areas without signal still contributes to which features we improve. Nothing you type is queued.' },
                    ...
                    { heading: '9. Your Rights', body: 'You may request access to, correction of, or deletion of any personal data we hold by contacting us via the Feedback button or at hello@sonobuddy.com.\n\nUsage analytics are aggregated counts and are not linked to you, so there is no individual record to retrieve or erase. Feedback messages and any email address you provide can be deleted on request.' },
